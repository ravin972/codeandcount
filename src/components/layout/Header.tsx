...
import Image from 'next/image'; // Keep if used elsewhere, remove if only for dropdown
...
export function Header() {
  ...
  return (
    ...
    >
      <div className={cn(
        "transition-all duration-300 ease-in-out p-3 md:p-4"
      )}> 
        <div
          className={cn(
        ...
        >
          ...
          <nav className={cn(
            "hidden md:flex flex-grow justify-center items-center transition-all duration-300 ease-in-out space-x-1 lg:space-x-2"
            )}>
            {mainNavLinks.map((link) => (
              <Link
                ...
                href={link.href}
                className={cn(
                  "text-sm font-medium transition-all duration-300 ease-in-out hover:text-primary rounded-md relative px-2.5 py-1.5",
          ...
          <div className={cn(
            "hidden md:flex items-center transition-all duration-300 ease-in-out space-x-3 lg:space-x-4"
            )}>
            <Button
              variant="default" 
              size="default"
              className="rounded-full relative pl-4 pr-10 h-9"
              onClick={() => router.push('/contact#start-project')}
            >
              <span>Start a project</span>
              <ArrowUpRight
                className="absolute top-1/2 -translate-y-1/2 right-2.5 h-4 w-4"
              />
            </Button>
            <ThemeToggle />
          </div>
