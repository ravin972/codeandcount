...
const calculateSlightlyDifferentColor = (baseHex: string, level: number): string => {
  ...
  // Difficulty factor: smaller difference for higher levels
  // Max difference of 30, min difference of ~5
...
export default function HexTestPage() {
  ...
  const setupNewLevel = useCallback(() => {
    ...
    setTimeLeft(INITIAL_TIME_PER_LEVEL + Math.max(0, 5 - Math.floor(level/3))); // Slightly more time for early levels, less for later
  ...
  const handleHexagonClick = (index: number) => {
    ...
    if (index === correctHexIndex) {
      ...
      setLevel((l) => l + 1);
      // setupNewLevel() will be called by the useEffect listening to level change
      setMessage({text: "Correct!", type: 'success'});
    } else {
      setIsGameOver(true);
      setMessage({text: "Wrong choice! Game Over.", type: 'error'});
    }
  };
  
  const hexagonSize = 100 / getGridDimensions() - (getGridDimensions() > 3 ? 2 : 1) ; // percentage, with gap reduction for more items
    ...
    <div className="bg-background text-foreground min-h-screen py-8">
      <header className="text-center mb-12">
        <h1 className="text-4xl md:text-5xl font-bold tracking-tight flex items-center justify-center">
          <Palette className="h-10 w-10 mr-3 text-primary" />
          Hex Color Challenge
        </h1>
        ...
        {!gameStarted || isGameOver ? (
          <Card className="shadow-xl text-center">
            <CardHeader>
              ...
              {gameStarted && isGameOver && <CardDescription className="text-xl">Your final score: {score}</CardDescription>}
            </CardHeader>
            <CardContent>
              <p className="text-muted-foreground mb-6">
                {isGameOver && gameStarted ? message?.text : "Click the hexagon that has a slightly different color. Each level gets harder. Good luck!"}
              </p>
          ...
          <Card className="shadow-xl">
            <CardHeader>
              <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-4">
                ...
                <Badge variant={timeLeft <=5 ? "destructive" : "outline"} className="text-lg p-2">
                ...
                {gridColors.map((color, index) => (
                  <div
                    ...
                    style={{
                      ...
                      paddingBottom: '100%', // Creates a square aspect ratio for the cell
                      backgroundColor: color,
                      clipPath: 'polygon(50% 0%, 100% 25%, 100% 75%, 50% 100%, 0% 75%, 0% 25%)', // CSS Hexagon
                      transform: 'scale(0.95)', // To ensure borders don't overlap harshly with clip-path
                    }}
